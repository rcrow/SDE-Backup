import arcpy
import datetime
import pandas #requires at least 0.21
import smtplib

#INPUTS
#!NOTE! the path to the following file needs to be changed for other system configurations
excelFile = r"\\igswzcwwgsrio\loco\Team\Crow\_Python\SDEbackup\backupParameters_106.xlsx"
...
def sendEmail(fromaddr,toaddrs,msg):
    ...
    EmailParams = pandas.read_excel(excelFile, sheet_name='email')
    print(EmailParams)
    username=EmailParams.iat[0,0]
    password=EmailParams.iat[0,1]
    print(username)
    print(password)
...
for i in range(0,len(servers)):
    ...
    arcpy.env.workspace = connectionPath + "\\" + "ConnectForBackupTo_" + servers[i].split(".")[0] +"_"+ dbs[i]+".sde"
